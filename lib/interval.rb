class Interval
  ...
  def apply(star_mappings, graphmaster, predicates, category_tree)
    ...
    if (from && to)
      formatted_from = from.apply(star_mappings, graphmaster, predicates, category_tree)
      formatted_to = to.apply(star_mappings, graphmaster, predicates, category_tree)
      interval = style ? style.apply(star_mappings, graphmaster, predicates, category_tree) : 'years'
