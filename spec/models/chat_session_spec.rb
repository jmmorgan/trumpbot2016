...
RSpec.describe ChatSession, type: :model do
  ...
  describe '#chat_json' do
    ...
    it 'supports serializing/deserializing a Chat' do
      ...
      chat_session_out = ChatSession.find_by(session_id: 'foo')
      expect(chat_in.requests).to eq chat_session_out.requests
      expect(chat_in.responses).to eq chat_session_out.responses
    end
  end
